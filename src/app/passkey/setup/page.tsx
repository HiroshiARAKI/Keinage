...
export default async function PasskeySetupPage({
  ...
  if (!session.webauthnVerified && (await countWebAuthnCredentials(session.user.id)) > 0) {
    redirect(
      redirectTo
        ? `/passkey/verify?redirectTo=${encodeURIComponent(redirectTo)}`
        : "/passkey/verify",
    );
  }

  return <PasskeyClient mode="register" redirectTo={redirectTo} />;
}
