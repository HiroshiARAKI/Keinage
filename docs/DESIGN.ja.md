...
`src/lib/weather/types.ts` でプロバイダ非依存の予報モデルとプロバイダI/Fを定義します。外部API固有のレスポンス解析は `src/lib/weather/providers/` 以下へ閉じ込め、現在の `tsukumijima` アダプタは日本語の予報データを共通の天気状態コード、摂氏気温、4区分の降水確率へ変換します。

`src/lib/weather/service.ts` はプロバイダ・観測地単位で予報を30分ごとに更新します。同じ予報日では各更新の非null値を日次キャッシュへマージし、後続レスポンスから最高気温や経過済み時間帯の降水確率が欠落しても、取得済みの値を消しません。予報日が変わった場合は新しい日次キャッシュへ切り替えます。同じ未キャッシュ地点への同時リクエストは1回の外部通信へ集約します。一度取得した地点の更新に失敗した場合は、表示中のサイネージから天気が消えないようstaleデータを返します。`/api/weather` はボード認可とOwner設定の観測地解決を担当し、外部アクセスはすべてこのサービスへ委譲します。
...
| 天気プロバイダ（既定 `tsukumijima`） | 正規化した天気予報データ |
