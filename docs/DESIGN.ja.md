...
`src/lib/weather/service.ts` はプロバイダ・観測地単位で予報を30分キャッシュします。同じ未キャッシュ地点への同時リクエストは1回の外部通信へ集約します。一度取得した地点の更新に失敗した場合は、表示中のサイネージから天気が消えないようstaleデータを返します。`/api/weather` はボード認可とOwner設定の観測地解決を担当し、外部アクセスはすべてこのサービスへ委譲します。
