...
表示画面は約5分間隔で heartbeat を送信します。管理者は `/status` で、表示端末ごとの最終アクセス日時、表示中ボード、User-Agent、5分以内アクセスをもとにした Online 表示を確認できます。この端末状態表示は Self-hosted / Unlimited、および Lite 以上のプランで利用できます。端末名の手動設定、IPアドレス保存、長期表示履歴、Proof of Play は対象外です。
