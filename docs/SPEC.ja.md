...
天気表示は全ボード共通の内部プロバイダを利用します。予報は観測地単位で30分ごとに更新・再キャッシュするため、複数ボードが外部APIを個別に呼び出しません。画面は外部プロバイダの画像ではなく、Keinage共通のモノクロ天気アイコンを使用します。横長カードにはOwner設定の観測地、今日の天気、最高・最低気温、0-6時・6-12時・12-18時・18-24時の降水確率を表示します。対象地域は admin が設定できます。
